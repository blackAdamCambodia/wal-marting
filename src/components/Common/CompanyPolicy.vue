<script>

</script>
<template>
    <section class="bg-white border-top mt-6">
    <div class="container">
        <div class="row no-gutters">
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="/faq">
                    <i class="fa fa-question-circle fa-3x text-primary mb-2"></i>
                    <h4 class="h6">Frequently Asked Questions</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="/return-policy">
                    <i class="fa fa-mail-reply fa-3x text-primary mb-2"></i>
                    <h4 class="h6">return policy</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left text-center p-4 d-block" href="/about-company">
                    <i class="fa fa-support fa-3x text-primary mb-2"></i>
                    <h4 class="h6">About Company</h4>
                </a>
            </div>
            <div class="col-lg-3 col-md-6">
                <a class="text-reset border-left border-right text-center p-4 d-block" href="/privacy-policy">
                    <i class="fa fa-exclamation-circle fa-3x text-primary mb-2"></i>
                    <h4 class="h6">privacy policy</h4>
                </a>
            </div>
        </div>
    </div>
</section>
</template>